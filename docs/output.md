...
4. **Create consensus set of peaks**
    ...
    *Output directories*:
    * `bwa/mergedLibrary/macs/consensus/`  
...
5. **Read counting and differential accessibility analysis**
    ...
    *Output directories*:
    * `bwa/mergedLibrary/macs/consensus/deseq2/`
    ...
    * `bwa/mergedLibrary/macs/consensus/<COMPARISON>/`  
    ...
    * `bwa/mergedLibrary/macs/consensus/sizeFactors/`  
...
1. **Reference genome files**
    ...
    * `reference_genome/BWAIndex/`  
      If they dont exist already and if the `--saveReference` parameter is provided then the alignment indices generated by the pipeline will be saved in this directory. This can be quite a time-consuming process so it permits their reuse for future runs of the pipeline or for other purposes.  
