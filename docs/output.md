...
6. **Read counting and differential binding analysis**
    ...
    *Description*:  
    Differential binding analysis, PCA and clustering using reads quantified at the replicate-level for the merged peak set.
...
2. **Create IGV session file**
    ...
    *Description*:  
    ...
    The path to the genome fasta file provided to the pipeline will be set as the genome for the IGV session. If you prefer to use an in-built genome provided by IGV just change the file path in the second-line of the session file to the name of the IGV genome e.g. mm10 or hg19.
