...
5. **Create consensus set of peaks**
    ...
    * `bwa/replicate/macs2/consensus/`  
      ...
      * Spreadsheet representation of consensus peak-set across samples **without** gene annotation columns: `*.boolean.txt`. Use file above for downstream analysis.  
...
2. **Create IGV session file**
    ...
    *Description*:  
    An IGV session file  will be created at the end of the pipeline containing paths to the normalised bigWig tracks, peaks and differential sites generated by the pipeline. This avoids having to load all the data individually into IGV for visualisation.
...
File paths in the IGV session file will be set as absolute paths to the directory containing the results.  
-->
