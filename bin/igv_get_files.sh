...
## CONSENSUS PEAKS
find -L $INPUT_DIR -type f -iname "merged*$FILE_GROUP*" | cut -c 2- | awk -v OFS='\t' '{ print "RESULTS_DIR"$1, "0.0.0" }'
...
find -L $INPUT_DIR -type f -iname "*$FILE_GROUP*" | grep -Ev "merged|FDR" | sort | cut -c 2- | awk -v OFS='\t' '{ print "RESULTS_DIR"$1, "0,0,178" }'
